...
import { messages, posts, reports, userBlocks, users } from "../drizzle/schema";

describe("UGC safety schema", () => {
  it("shares the report status contract with the database schema", () => {
    expect(REPORT_STATUSES).toEqual(["pending", "action_taken", "dismissed"]);
  ...
  it("exposes report and block tables", () => {
    ...
    expect(reports.targetType.enumValues).toEqual(["post", "message", "user"]);
